  ...
  return (
    <div className="courses-page min-h-screen bg-[#0a0a0a] text-white">
      <main className="mx-auto max-w-[1760px] px-4 py-6 md:px-6 xl:px-8">
                ...
                <article className="courses-category-card flex h-full min-h-[9.5rem] flex-col items-center rounded-xl border border-white/10 bg-linear-to-br from-white/[0.06] to-black/35 p-3 text-center transition hover:border-amber-300/40 hover:bg-white/5">
                  <div className="relative mx-auto mb-2 h-10 w-10 shrink-0">
                    {style.imageSrc ? (
                      ...
                      <div
                        className={`courses-icon-chip inline-flex h-10 w-10 items-center justify-center rounded-full border ${style.iconBorder} ${style.iconBg} ${style.iconTone}`}
                      >
                        <Icon size={16} />
                      </div>
                    )}
                    <span
                      className={`courses-category-badge courses-icon-chip absolute -bottom-1 -right-1 inline-flex h-5 w-5 items-center justify-center rounded-full border border-black/20 bg-[#0b0b0b]/85 ${style.iconTone}`}
                    >
                      <Icon size={11} />
                    </span>
                  </div>
        ...
        <section className="mt-4 grid gap-4 lg:grid-cols-2">
          <article className="courses-featured-panel rounded-2xl border border-amber-300/35 bg-linear-to-br from-[#0b1530] via-[#091224] to-[#050a16] p-5 shadow-[0_0_26px_rgba(245,158,11,0.2)]">
              ...
              <div className="flex items-center">
                {featuredExperts.map((expert, idx) => (
                  <div key={expert.name} className={`group relative ${idx > 0 ? "-ml-3" : ""}`}>
                    ...
                    <span className="courses-expert-tooltip pointer-events-none absolute -bottom-6 left-1/2 hidden -translate-x-1/2 whitespace-nowrap rounded-md border border-amber-300/40 bg-[#0a1224] px-2 py-0.5 text-[10px] text-amber-100 group-hover:block">
                      {expert.name}
                    </span>
                  </div>
                ))}
          ...
          <article className="courses-surface rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            ...
            <div className="space-y-2">
              {cpConfig.faqs.map((faq) => (
                <div key={faq.question} className="courses-faq-item rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm">
                  {faq.question}
                </div>
              ))}
            </div>
        ...
        <section className="courses-cta-banner mt-4 overflow-hidden rounded-2xl border border-amber-300/30 bg-linear-to-r from-[#7a4f00]/40 via-[#241400] to-[#7a4f00]/30">
