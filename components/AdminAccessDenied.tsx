...
export default function AdminAccessDenied({ userEmail, message }: Props) {
  return (
    <div className="flex min-h-[70vh] items-center justify-center px-6 py-16">
      <div className="max-w-md rounded-2xl border border-amber-500/30 bg-black/50 p-8 text-center shadow-xl">
        ...
        <p className="mt-3 text-sm leading-relaxed text-zinc-300">
          {message ??
            "Only the authorized administrator can open this panel after signing in."}
        </p>
        {userEmail ? (
          <p className="mt-2 text-xs text-zinc-500">Signed in as: {userEmail}</p>
        ) : null}
